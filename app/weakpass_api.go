...
//	PUT  /api/authcheck/config  增删目标白名单(整体替换, admin) —— 写 settings.json
//	                            的 authcheck 节并热生效, 免重启
...
// ===== 目标白名单管理(增删, 写 settings.json + 热生效) =====

const authCheckMaxTargets = 200 // 白名单条数上限(防一次请求把巨量列表写进 settings.json)

// handleAuthCheckConfig PUT /api/authcheck/config  body: {"targets": ["192.168.1.0/24", "10.0.0.5"]}
//
// 弱口令的目标白名单此前只能手改 settings.json 再重启 —— 用户找不到入口。
// 这里做成接口: 整体验证(每项必须是合法 IP 或 CIDR, 拒绝"192.168.1.*"这类
// 宽泛写法)→ 合并写回 authcheck 节(enabled/dictFile/rate 等其它字段原样保留)
// → 热替换引擎配置, 立即生效免重启。
//
// 语义 = 整体替换而非逐条增删: 白名单是合规红线(空 = 拒绝一切), 逐条改的
// 中间态不可见; 整体替换让"生效瞬间"唯一, 前端拿当前列表增删后整份提交。
// 清空(提交空列表)是合法的"全部禁止"操作, 审计留痕。
func handleAuthCheckConfig(w http.ResponseWriter, r *http.Request) {
	...
	targets, err := validateAuthCheckTargets(req.Targets)
	if err != nil {
		jsonErr(w, http.StatusBadRequest, err.Error())
		return
	}

	// 读现有 authcheck 节, 只换 targets, 其它字段原样保留(合并写, 与
	// persistAICfg 同口径: 用户手写的字段不能被本次保存抹掉)
	var cur AuthCheckConfig
	if raw, ok := section(secAuthCheck, ""); ok {
		if uerr := json.Unmarshal(raw, &cur); uerr != nil {
			jsonErr(w, http.StatusInternalServerError,
				"settings.json 的 authcheck 节解析失败, 请手工检查: "+uerr.Error())
			return
		}
	}
	cur.Targets = targets
	if err := writeSection(secAuthCheck, cur); err != nil {
		jsonErr(w, http.StatusInternalServerError, "写入 settings.json 失败: "+err.Error())
		return
	}
	resetSettingsCache()

	// 热生效: 整体替换引擎配置(与写盘同值)。instanceAuthCheck 保证单例存在 ——
	// 未启用时引擎同样持有配置对象, 用户随后启用时新白名单即已就位, 不会
	// "启用后白名单还是旧的"。
	e := instanceAuthCheck()
	e.SetConfig(cur.toWeakpass(authCheckExeDir()))
	authCheckMu.Lock()
	authCheckCfg = cur
	authCheckMu.Unlock()

	logAudit(v2DB(), r, "authcheck.config", "",
		"更新弱口令目标白名单: "+strings.Join(targets, ", "))
	logLine("弱口令白名单已更新: " + strings.Join(targets, ","))
	jsonOK(w, map[string]any{"ok": true, "targets": targets, "count": len(targets)})
}

// validateAuthCheckTargets 清洗并校验白名单(去空白/去重/限量 + 每项必须是
// 合法 IP 或 CIDR)。返回 (规范化列表, 错误)。
//
// 为什么逐条硬校验: 白名单是攻击面的边界, 一条"192.168.1.*"或"192.168.1"
// 这类宽泛/非法条目会静默扩大爆破范围 —— 校验失败必须整体拒绝(400),
// 而不是"跳过坏条目继续写", 那样用户以为改成功了, 实际防线少了一道。
func validateAuthCheckTargets(list []string) ([]string, error) {
	if len(list) > authCheckMaxTargets {
		return nil, fmt.Errorf("白名单条数上限 %d, 当前 %d", authCheckMaxTargets, len(list))
	}
	out := make([]string, 0, len(list))
	...
	for _, raw := range list {
		...
		if net.ParseIP(s) == nil {
			if _, _, perr := net.ParseCIDR(s); perr != nil {
				return nil, fmt.Errorf("非法白名单条目(须为 IP 或 CIDR, 如 192.168.1.10 或 192.168.1.0/24): %s", s)
			}
		}
		if seen[s] {
			continue
