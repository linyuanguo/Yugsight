...
// Test2FALazyRowFirstLoginAdmin 2026-09-24 真机踩坑回归: 2FA 懒创建行不得把
// 旧管理员锁进只读角色。
//
...
// ② 旧版本二进制遗留的坏行(role=auditor + 空哈希), 升级后首次登录回填时对齐
// 为 admin —— 否则存量机器升级后还得手工改 users.jsonl 才能解锁。
func Test2FALazyRowFirstLoginAdmin(t *testing.T) {
	d := inject2FADB(t)
	resetAccountMgr(t, d)
	resetAuth(t)
	resetTrustTokens()
	...
	// 场景一: v2 表无行的旧账号, 拉码值懒创建
	postJSON(t, handleRegister, `{"user":"bob","pass":"secret123","pass2":"secret123"}`)
	if _, err := d.Users().Delete("bob"); err != nil {
		t.Fatalf("构造'v2 表无行'场景失败: %v", err)
	}
	rec := httptest.NewRecorder()
	handle2FACode(rec, httptest.NewRequest(http.MethodGet, "/api/auth/2fa/code?user=bob", nil))
	var on struct {
		Enabled bool   `json:"enabled"`
		Code    string `json:"code"`
	}
	_ = json.Unmarshal(rec.Body.Bytes(), &on)
	if !on.Enabled || len(on.Code) != 6 {
		t.Fatalf("懒创建后应自动启用并返回 6 位码: %s", rec.Body.String())
	}
	u, err := d.Users().Get("bob")
	if err != nil || u == nil {
		t.Fatalf("懒创建行应存在: %v", err)
	}
	if u.Role != db.RoleAdmin {
		t.Fatalf("懒创建行角色必须是 admin(旧账号全权限), got %q", u.Role)
	}
	r := doLoginReq(t, `{"user":"bob","pass":"secret123","code":"`+on.Code+`"}`, nil)
	var lr struct {
		Role string `json:"role"`
	}
	_ = json.NewDecoder(r.Body).Decode(&lr)
	_ = r.Body.Close()
	if r.StatusCode != 200 || lr.Role != db.RoleAdmin {
		t.Fatalf("首次登录应拿到 admin 会话, got %d role=%q", r.StatusCode, lr.Role)
	}
}

// Test2FAStaleRowFirstLoginAdmin 场景二(2026-09-24): 旧版本二进制懒创建留下的
// 坏行(已存在 + role=auditor + 空哈希)升级后首次登录, 回填时须对齐 admin。
//
// 单独立用例的原因: 注册接口沿用旧单用户体系(一个实例只注册一个账号),
// 两个场景共享一个库会互相挤占。
func Test2FAStaleRowFirstLoginAdmin(t *testing.T) {
	d := inject2FADB(t)
	resetAccountMgr(t, d)
	resetAuth(t)
	resetTrustTokens()
	...
	postJSON(t, handleRegister, `{"user":"carol","pass":"secret123","pass2":"secret123"}`)
	cu, err := d.Users().Get("carol")
	if err != nil || cu == nil {
		t.Fatalf("读取 carol 行失败: %v", err)
	}
	// 模拟旧版懒创建留下的坏行: 角色落在默认 auditor + 密码未回填
	cu.Role = db.RoleAuditor
	cu.PassHash = ""
	if _, uerr := d.Users().Upsert(cu); uerr != nil {
		t.Fatalf("构造旧版坏行失败: %v", uerr)
	}
	// 拉码值: 行已存在, 只补种子(角色保持 auditor —— 正是旧版的遗留状态)
	rec2 := httptest.NewRecorder()
	handle2FACode(rec2, httptest.NewRequest(http.MethodGet, "/api/auth/2fa/code?user=carol", nil))
	var on2 struct {
		Code string `json:"code"`
	}
	_ = json.Unmarshal(rec2.Body.Bytes(), &on2)
	if on2.Code == "" {
		t.Fatalf("carol 应能拉到码值: %s", rec2.Body.String())
	}
	r2 := doLoginReq(t, `{"user":"carol","pass":"secret123","code":"`+on2.Code+`"}`, nil)
	var lr2 struct {
		Role string `json:"role"`
	}
	_ = json.NewDecoder(r2.Body).Decode(&lr2)
	_ = r2.Body.Close()
	if r2.StatusCode != 200 || lr2.Role != db.RoleAdmin {
		t.Fatalf("旧版坏行首次登录应回填并提升为 admin, got %d role=%q", r2.StatusCode, lr2.Role)
	}
	cu2, _ := d.Users().Get("carol")
	if cu2 == nil || cu2.Role != db.RoleAdmin || cu2.PassHash == "" {
		t.Fatalf("登录后库内行应已对齐 admin + 非空哈希: %+v", cu2)
	}
}
