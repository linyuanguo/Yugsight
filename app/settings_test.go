...
// TestSettingsLoadLogOnlyOnce 守"配置重载不刷日志"契约。
//
// 写配置后必须 resetSettingsCache(否则读到过期值 —— NVD 续传进度刚写进
// Next=57、缓存里还是 56, 下次续传判断直接错位), 但那之后的重载若每次都打
// "已加载", 高频写配置的场景就会刷屏: 2026-09-25 实测 NVD 分批同步 109 批 =
// 109 行同样的 "settings.json 已加载", 让人以为是配置被反复读写出问题。
func TestSettingsLoadLogOnlyOnce(t *testing.T) {
	withTempExeDir(t)
	writeTestSettings(t, `{"nvd":{"apiKey":"LOG-ONCE"}}`)

	// 把全局日志临时指向临时文件, 直接数"已加载"出现的次数
	dir := t.TempDir()
	path := filepath.Join(dir, "log-once.log")
	old := logOut
	logOut = newLogWriter(path, dir, 1<<20, 3)
	defer func() {
		logOut.Close()
		logOut = old
	}()
	settingsMu.Lock()
	settingsLogged = false
	settingsMu.Unlock()
	defer func() {
		settingsMu.Lock()
		settingsLogged = false
	...
	if err != nil {
		t.Fatalf("读回日志失败: %v", err)
	}
	if n := strings.Count(string(data), settingsFileName+" 已加载"); n != 1 {
		t.Fatalf("三次加载应只打一次'已加载'日志, 实际 %d 次\n%s", n, data)
	}
}
