...
//   exe 同目录 globe/), 运行时经 /vendor/globe/{file} 直读, 不 go:embed 进二进制
...
func instanceGeoIP() *geoip.DB {
	...
	build := func() {
		...
		dir := filepath.Join(exeDir(), "geoip")
...
// globeDir 运行时 globe 资源目录(exe 同目录 globe/, 由 build.ps1 从 build/globe 镜像)。
func globeDir() string {
	return filepath.Join(exeDir(), "globe")
}

// handleGlobeFile GET /vendor/globe/{file} 直读 exe 同目录 globe/ 下文件。
//
