...
// word/html/pdf 三种格式; 模板文件放 exe 同目录 report_templates/(外部资源,
...
type ReportConfig struct {
	...
	// 模板包(会写 exe 同目录 report_templates/)。指针区分"未配置"与显式 false,
	// 默认关 —— 目录写权限不该默认开放。
...
type reportRequest struct {
	...
	// WordTemplate Word 模板名(exe 同目录 report_templates/ 下的 .docx, 不含扩展名;
	// 空 / "builtin" = 内置模板)
...
// 空 / "builtin" = 内置模板; 其它 = exe 同目录 report_templates/<name>.docx。
...
// wordTplDir Word 模板目录(exe 同目录 report_templates/; 声明为变量便于测试
...
var wordTplDir = func() string {
	...
	return filepath.Join(filepath.Dir(exe), "report_templates")
}
