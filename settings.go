...
func settingsSectionNames(m map[string]json.RawMessage) []string {
	...
	order := []string{secEngine, secAI, secCapture, secUpdater, secProbe,
		secScheduler, secReport, secScreen, secDatabase, secAuth, secWhitelist, secAuthCheck, secTLS, secAudit, secMonitor, secDashboard, secGeoIP}
...
// resetSettingsCache 清空配置缓存(仅供测试)。
//
