...
//   2. 官方扩展包(official): exe 同目录 rules/ 目录(递归, 不含 custom/.staging 子目录)
//   3. 用户自定义包(custom): exe 同目录 rules/custom/ 目录(优先级最高)
//
// 核心行为:
...
const (
	...
	RuleTierOfficial = "official" // 二级: 官方扩展包(exe 同目录 rules/)
	RuleTierCustom   = "custom"   // 三级: 用户自定义包(exe 同目录 rules/custom/, 最高优先级)
)

// ===== 目录定位(测试可覆盖) =====

// rulesExternalDir 测试用目录覆盖(空 = exe 同目录 rules/)
var rulesExternalDir = ""

// rulesOfficialDir 官方扩展包目录(默认 exe 同目录 rules/)
