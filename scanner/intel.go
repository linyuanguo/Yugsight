...
//   - 外部热更新: exe 同目录 intel/*.json(同格式, 按 CVE 号覆盖内置条目),
...
// intelExternalDir 测试用目录覆盖(空 = exe 同目录 intel/)
var intelExternalDir = ""

// intelDir 外部情报目录(默认 exe 同目录 intel/, 可选, 不存在则只用内置)
func intelDir() string {
	if intelExternalDir != "" {
		return intelExternalDir
	}
	if exe, err := os.Executable(); err == nil {
		return filepath.Join(filepath.Dir(exe), "intel")
	}
	return "intel"
}
