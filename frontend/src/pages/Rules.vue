    ...
    <div class="card">
      ...
      <div style="margin-top:10px" v-if="cpeProducts">
        <button class="btn sm" :disabled="sync.running || syncBusy" @click="startSync"
                title="从 NVD 拉取最新 CVE 数据扩充主机 CPE 规则库(需网络可达 services.nist.gov)">
        ...
        <span class="muted small" v-if="sync.running" style="margin-left:12px">
          ...
          · 累计 {{ sync.totalCves }} 条 CVE, 命中 {{ sync.keptCves }}
        </span>
    ...
    <div class="card">
      ...
      <div class="toolbar" style="flex-wrap:wrap; gap:10px">
        <span class="chip" :class="upd.running ? 'warn' : (upd.available ? 'blue' : 'on')">
          ...
          {{ upd.running ? '更新进行中' : (upd.available ? '有新版本可更新' : '已是最新') }}
        </span>
        ...
        <button class="btn sm" :disabled="updBusy || upd.running" @click="startUpdate('rules')"
                title="从 updater.sources 配置的自建下载源更新(源端需带 rules-manifest.json)">
        ...
        <table class="table">
          ...
          <tbody>
            <tr v-for="r in visibleRows" :key="r.id">
              ...
              <td class="mono small" style="word-break:break-all; max-width:280px">{{ r.pattern }}</td>
              <td class="small">
...
async function loadLastSync() {
  try {
    ...
    if (p && p.finishedAt) lastSync.value = fmtDT(p.finishedAt)
  } catch (e) { /* 静默: 展示项, 失败不影响页面主体 */ }
}

// ===== NVD 一键同步 =====
const sync = ref({ running: false, finished: false, currentPage: 0, totalPages: 0, totalCves: 0, keptCves: 0, products: 0, error: '' })
const syncBusy = ref(false)
let syncTimer = null

async function startSync() {
  if (sync.value.running) return
  if (!confirm('将从 NVD (services.nist.gov) 拉取全量 CVE 数据, 过滤到可指纹产品后写入 cpe/ 目录。\n\n' +
    '注意: 需网络可达 NVD(国内可能需要代理)。全量约 25 万条 CVE, 耗时约 5-10 分钟。\n\n继续?')) return
  syncBusy.value = true
  sync.value = { running: true, finished: false, currentPage: 0, totalPages: 0, totalCves: 0, keptCves: 0, products: 0, error: '' }
  try {
    await api('/api/vuln/rules/sync', { method: 'POST', body: { minCvss: 0 } })
    startSyncPoll()
  } catch (e) {
