...
// DefaultDataDir 默认数据目录: exe 同目录 data/scanctl/
// (白名单 whitelist.jsonl / 误报 fps.jsonl 存于此; 路径可被测试覆盖)。
// 2026-09-24 dist 目录整理: 本地扫描数据统一收进 data/(旧 "exe 同目录 scanctl/"
// 由 main.go 的 migrateLegacyDirs 启动时一次性迁移)。
func DefaultDataDir() string {
	if exe, err := os.Executable(); err == nil {
		return filepath.Join(filepath.Dir(exe), "data", "scanctl")
	}
	return "data/scanctl"
}
