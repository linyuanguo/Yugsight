...
//   - 内置 cpe_builtin.json 编译进 exe(//go:embed)作兜底; exe 同目录 vuln/cpe/*.json 热更新:
...
// cpeExternalDir 外部 CPE 字典目录覆盖(空 = exe 同目录 vuln/cpe/), 测试用
// cpeExternalDir 测试用目录覆盖(空 = exe 同目录 vuln/cpe/)
var cpeExternalDir = ""

// cpeDir CPE 外部规则目录。
// 2026-09-24 dist 目录整理: 规则库统一收进 vuln/(要分发的只有一种 —— 规则库),
// 故外部 CPE 目录从 "exe 同目录 cpe/" 迁到 "exe 同目录 vuln/cpe/"。
// 旧目录由 main.go 的 migrateLegacyDirs 在启动时一次性迁移。
func cpeDir() string {
	if cpeExternalDir != "" {
		return cpeExternalDir
	}
	if exe, err := os.Executable(); err == nil {
		return filepath.Join(filepath.Dir(exe), "vuln", "cpe")
	}
	return "vuln/cpe"
}
